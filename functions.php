...
require_once('library/hla.php');            // core functions (don't remove)
...
add_filter('admin_footer_text', 'hla_custom_admin_footer');
function hla_custom_admin_footer() {
 ...
 */
function hla_filter_title( $title, $sep ) {
	global $paged, $page;
...
add_filter( 'wp_title', 'hla_filter_title', 10, 2 );
...
add_image_size( 'featured-home', 340, 250, true);
add_image_size( 'carousel', 1130, 480, true);
add_image_size( 'banner', 1130, 160, true);
...
function hla_register_sidebars() {
...
// Comment Layout
function hla_comments($comment, $args, $depth) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			<div class="comment-author vcard row-fluid clearfix">
				...
				<div class="span9 comment-text">
					...
					<?php edit_comment_link(__('Edit hlatheme'),'<span class="edit-comment btn btn-small btn-info"><i class="icon-white icon-pencil"></i>','</span>') ?>
                    
                    <?php if ($comment->comment_approved == '0') : ?>
       					<div class="alert-message success">
          				<p><?php _e('Your comment is awaiting moderation.','hlatheme') ?></p>
          				</div>
					<?php endif; ?>
...
// Search Form
function hla_wpsearch( $form ) {
  $form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
  <label class="screen-reader-text" for="s">' . __('Search for:', 'hlatheme') . '</label>
  <input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="Search the Site..." />
  <input type="submit" id="searchsubmit" value="'. esc_attr__('Search','hlatheme') .'" />
  </form>';
  return $form;
} // don't remove this bracket!
...
function custom_password_form() {
	...
	' . '<p>' . __( "This post is password protected. To view it please enter your password below:" ,'hlatheme') . '</p>' . '
	<label for="' . $label . '">' . __( "Password:" ,'hlatheme') . ' </label><div class="input-append"><input name="post_password" id="' . $label . '" type="password" size="20" /><input type="submit" name="Submit" class="btn btn-primary" value="' . esc_attr__( "Submit",'hlatheme' ) . '" /></div>
	</form></div>
	';
	return $o;
}
